...
import DialCryptex from './components/DialCryptex';
...
const HOW_TO_PLAY_KEY = 'dial-how-to-play-seen';
...
export default function App() {
  ...
  // Let the wheels visibly lock into place (staggered color animation in
  // DialCryptex) before the win screen covers them — solving shouldn't
  // instantly cut to a share modal.
  useEffect(() => {
    if (gameStatus !== 'won') { setRevealWin(false); return; }
    const t = setTimeout(() => setRevealWin(true), 900);
  ...
  useEffect(() => {
    if (gameStatus === 'won') {
      ...
      recordTodayShare('dial', dateKey, generateShareText());
    }
  }, [gameStatus]);
  ...
  const Logo = () => (
    <h1 className={styles.logo}>
      <GameLogo />
      <span className={styles.logoText}>Dial</span>
    </h1>
  );
  ...
  return (
    <div className={styles.container}>
      ...
      {gameStatus === 'won' && revealWin && !winDismissed && (
        <WinScreen
          puzzle={puzzle}
