    ...
    <div className={`${styles.overlay} ${showContent ? styles.visible : ''}`}>
      <div className={styles.modal}>
        <div className={styles.sparkContainer} aria-hidden="true">
          {Array.from({ length: puzzle.n }).map((_, i) => (
            <span key={i} className={styles.spark} style={{
              left: `${10 + i * (80 / Math.max(puzzle.n - 1, 1))}%`,
              top: `${Math.random() * 100}%`,
              background: i % 2 === 0 ? '#0ea5e9' : '#38bdf8',
              boxShadow: '0 0 4px #0ea5e9, 0 0 8px #0ea5e988',
        ...
        <div className={styles.metricsRow}>
          ...
          <div className={styles.metric}>
            <span className={styles.metricValue}>{puzzle.n}</span>
            <span className={styles.metricLabel}>Letters</span>
          </div>
          <div className={styles.metricDivider} />
          <div className={styles.metric}>
            <span className={styles.metricValue}>{puzzle.k}</span>
            <span className={styles.metricLabel}>Per Wheel</span>
          </div>
        </div>
